...
async function moderateDelete(id, rowEl) {
  ...
  try {
    const { error } = await supabaseClient.schema("cadastral").from("engineer_messages").delete().eq("id", id);
    if (error) throw error;
    rowEl.remove();
  } catch (err) {
...
function wireForm() {
  document.getElementById("chatForm").addEventListener("submit", async (e) => {
    ...
    try {
      const { error } = await supabaseClient.schema("cadastral").from("engineer_messages").insert({
