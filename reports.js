...
async function checkSubscription(userId) {
  const REPORTS_INCLUDED_PLANS = ["monthly", "annual", "institution"];
  let data = null;
  try {
    const result = await supabaseClient
      .from("profiles")
      .select("is_subscribed, subscription_expires_at, subscription_plan")
      .eq("id", userId)
      .single();
    ...
    const planIncludesReports = REPORTS_INCLUDED_PLANS.includes(String(data.subscription_plan || "").toLowerCase());
    isSubscribed = !!(data.is_subscribed && notExpired && planIncludesReports);
  } catch (err) {
  ...
  if (badge) {
    const onWeeklyOnly = !isSubscribed && String(data?.subscription_plan || "").toLowerCase() === "weekly" && !!data?.is_subscribed;
    badge.textContent = isSubscribed
      ? "✓ Subscribed — full access"
      : onWeeklyOnly
        ? "Weekly plan — upgrade to Monthly for full reports"
        : "Free plan — 40% preview";
    badge.style.color = isSubscribed ? "var(--accent)" : "var(--muted)";
  }
  ...
  const planIncludedNote = document.getElementById("planIncludedNote");
  const currentPlan = String(data?.subscription_plan || "").toLowerCase();

  if (subCardStatus && standaloneBtn) {
    if (isSubscribed) {
      subCardTitle.textContent = `${currentPlan.charAt(0).toUpperCase()}${currentPlan.slice(1)} plan — active`;
      subCardStatus.textContent = data?.subscription_expires_at
        ? `Full report access until ${new Date(data.subscription_expires_at).toLocaleDateString()}.`
        : "Full access to every report.";
      standaloneBtn.style.display = "none";
      planIncludedNote.textContent = "";
    } else if (currentPlan === "weekly") {
      subCardTitle.textContent = "Weekly plan — active";
      subCardStatus.textContent = "Your Weekly plan doesn't include full Reports access.";
      standaloneBtn.textContent = "Upgrade to Monthly →";
      standaloneBtn.style.display = "";
      planIncludedNote.innerHTML = "Reports are included in <b>Monthly</b>, <b>Annual</b>, and <b>Institution</b> plans.";
    } else {
      subCardTitle.textContent = "Your Subscription";
      subCardStatus.textContent = "You're on the free tier — unlock every report with a paid plan.";
      standaloneBtn.textContent = "View plans →";
      standaloneBtn.style.display = "";
      planIncludedNote.innerHTML = "Reports are included in <b>Monthly</b>, <b>Annual</b>, and <b>Institution</b> plans.";
    }
  }
      ...
      <div class="locked-wrap">
        ...
        <div class="paywall-overlay">
          <div class="paywall-card">
            ...
            <p class="fine">Included in Monthly, Annual, and Institution plans</p>
            <button class="btn-solid subscribe-btn">View plans →</button>
          </div>
        </div>
      </div>`;
  }
  ...
  return `
    <div class="report-row">
      ...
      <div style="display:flex;align-items:center;gap:8px;">
        ${landUse ? `<span class="score-pill" style="background:rgba(34,224,171,.15);color:var(--accent);">${escapeHTML(landUse)}</span>` : ""}
        <button class="save-report-btn" data-gid="${row.gid ?? ""}" data-upi="${escapeHTML(row.upi || "")}" data-district="${escapeHTML(row.district || "")}" data-sector="${escapeHTML(row.sector || "")}" data-cell="${escapeHTML(row.cell || "")}" data-landuse="${escapeHTML(row.land_use || "")}" style="background:transparent;border:1px solid var(--border);color:var(--muted);border-radius:8px;padding:5px 9px;font-size:11px;cursor:pointer;">❤ Save</button>
      </div>
    </div>`;
}
...
function scorePill(score) {
  let color = "var(--danger)", bg = "rgba(239,68,68,.15)";
  if (score >= 70) { color = "var(--accent)"; bg = "rgba(34,224,171,.15)"; }
...
function wirePaywallModal() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("save-report-btn")) {
      saveReportAsProperty(e.target);
    }
  });
}

async function saveReportAsProperty(btn) {
  const gid = btn.dataset.gid ? Number(btn.dataset.gid) : null;
  const upi = btn.dataset.upi || null;
  btn.disabled = true;
  ...
  try {
    ...
    const { error } = await supabaseClient.from("saved_properties").insert({
      ...
      parcel_gid: gid,
      upi,
      district: btn.dataset.district || null,
      sector: btn.dataset.sector || null,
      cell: btn.dataset.cell || null,
      land_use: btn.dataset.landuse || null,
    });
    if (error) throw error;
    btn.textContent = "✓ Saved";
  } catch (err) {
